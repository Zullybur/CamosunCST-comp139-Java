...
public class Elevator
{
    ...
    private int elevatorID, currentFloor, destination, topFloor, bottomFloor;
     ...
     */
    public Elevator(int numFloors, boolean hasThirteen) throws IllegalArgumentException
    {
     ...
     * @param hasThirteen is True if the system has a 13th floor, otherwise False
     */
    public Elevator(int numFloors, int numSubFloors, boolean hasThirteen) throws IllegalArgumentException
    {
    ...
    /**
     * Internal Use: create an array of FloorButton objects representing the
     ...
     * @param floorID is a floor that is accessible by the elevator
     */
    public void addFloor(int floorID) throws IllegalArgumentException
    {
        if (floorID > topFloor || floorID < bottomFloor)
        {
            throw new IllegalArgumentException();
    ...
    /**
     * Adds a floor to the destination list for the elevator.
     * @param floorID is a floor that is accessible by the elevator
     */
    private void addFloorToList(int floorID)
    {
        ...
        // as the new next destination
            ...
            destinationList.addDestination(destination, currentFloor);
            destination = floorID;
        } else if (motion == -1 && floorID < currentFloor && floorID > destination) {
            destinationList.addDestination(destination, currentFloor);
            destination = floorID;
        } else {
            destinationList.addDestination(floorID, currentFloor);
        }
     ...
     * Completes elevator motion if movement is warranted, and responds to the
     * new state once the location is updated.
     */
    ...
    private void arrived() {
        // TODO: tell the system to open the door and deactivate any call buttons as required based on elevator direction.
        // TODO: if the elevator is stationary, deactivating a call button should set it's direction!
        floorButtons[currentFloor].deactivate();
        innerDoor.open();
        playChime();
        destination = destinationList.getNextDestination(motion);
    }
}